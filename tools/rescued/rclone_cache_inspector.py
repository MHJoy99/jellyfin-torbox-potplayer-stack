#!/usr/bin/env python3
# RESCUED-FROM: archive/old-main-2026-08-21:tools/rclone_cache_inspector.py
# WHAT-CHANGED (cleaned for PUBLIC MIT repo):
#   - Added this header (origin + changes). No secrets were present; verified stdlib-only.
#   - Default --cache-dir is now $env:RCLONE_CACHE_DIR when set (fallback keeps old example path for compat).
#   - Added --DryRun/--dry-run/--WhatIf preview for ALL purge paths: prints what WOULD be purged/freed, deletes nothing.
#   - Purge guards unchanged (skip dirty + locked unless --force); DryRun never deletes even with --force.
#   - No new dependencies (stdlib only: os/sys/json/time/ctypes/argparse/datetime/pathlib/typing).
#   - Read-only by default (scan/tree/lru/check-locks); writes only via --purge-item/--purge-all (guarded by DryRun).
...
# Windows console safety (no emoji output, but keep UTF-8 where available).
try:
...
def main():
    ...
    parser.add_argument(
        "--cache-dir",
        default=os.environ.get("RCLONE_CACHE_DIR", r"F:\rclone-cache\gdrive-media"),
        help="Path to rclone cache directory (or set RCLONE_CACHE_DIR env; fallback example: F:\\rclone-cache\\gdrive-media)"
    )
    parser.add_argument(
        "--tree",
        action="store_true",
    ...
    parser.add_argument(
        ...
        dest="dry_run",
        action="store_true",
        help="Preview purge actions without deleting anything (safe preview for all writes)"
    )
    ...
    if args.dry_run and (args.purge_item or args.purge_all):
        # Safe preview: enumerate what WOULD be purged without deleting.
        preview_targets = []
        if args.purge_item:
            _norm = args.purge_item.replace("\\", "/").strip("/")
            for it in inspector.items:
                _rel = it.rel_path.replace("\\", "/")
                if _rel.startswith(_norm) or _norm in _rel:
                    if it.is_dirty and not args.force:
                        preview_targets.append((it.rel_path, "SKIP-DIRTY", it.effective_cached_size))
                    else:
                        locked, _msg = it.check_lock()
                        if locked and not args.force:
                            preview_targets.append((it.rel_path, "SKIP-LOCKED", it.effective_cached_size))
                        else:
                            preview_targets.append((it.rel_path, "WOULD-PURGE", it.effective_cached_size))
            print(f"[DryRun] Would purge {sum(1 for _, s, _ in preview_targets if s == 'WOULD-PURGE')} item(s) matching '{args.purge_item}' (Force: {args.force}). Deletes nothing.")
        else:
            _cutoff = None
            if args.older_than_days is not None:
                _cutoff = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=args.older_than_days)
            for it in inspector.items:
                if _cutoff and it.atime and it.atime > _cutoff:
                    continue
                if it.is_dirty and not args.force:
                    preview_targets.append((it.rel_path, "SKIP-DIRTY", it.effective_cached_size))
                    continue
                ...
                if locked and not args.force:
                    ...
                    preview_targets.append((it.rel_path, "WOULD-PURGE", it.effective_cached_size))
            _age = f" older than {args.older_than_days} days" if args.older_than_days else ""
            print(f"[DryRun] Would purge {sum(1 for _, s, _ in preview_targets if s == 'WOULD-PURGE')} item(s){_age} (Force: {args.force}). Deletes nothing.")
        _freed = sum(sz for _, s, sz in preview_targets if s == "WOULD-PURGE")
        print(f"[DryRun] Would free ~{format_bytes(_freed)} across preview set.")
        for _p, _s, _sz in preview_targets[:30]:
            print(f"  [{_s}] {_p} ({format_bytes(_sz)})")
        if len(preview_targets) > 30:
            print(f"  ... and {len(preview_targets) - 30} more.")
        if args.json:
            print(json.dumps({"dry_run": True, "preview": [{"path": p, "status": s, "bytes": sz} for p, s, sz in preview_targets]}, indent=2))
        return
