#!/usr/bin/env python3
# RESCUED-FROM: archive/old-main-2026-08-21:tools/media_quality_analyzer.py
# WHAT-CHANGED (cleaned for PUBLIC MIT repo):
#   - Added this header (origin + changes). No secrets were present; verified stdlib-only.
#   - Removed machine-specific docstring path; generalized description for public reuse.
#   - Added --DryRun/--dry-run/--WhatIf preview (lists files that WOULD be analyzed, invokes no ffprobe, writes nothing).
#   - Added --out-file <path> for JSON report; combined with --DryRun it only previews the write.
#   - --ffprobe-path now defaults to $env:FFPROBE_PATH when set (else auto-detect via shutil.which).
#   - No new dependencies (stdlib only: argparse/json/logging/os/shutil/subprocess/sys/dataclasses/pathlib/typing).
#   - Read-only by default: never modifies media files; only writes when --out-file is given (guarded by DryRun).
...
# Windows console safety: human report uses unicode markers; avoid cp1252 crashes.
try:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Media Quality & Stream Analyzer for Jellyfin / Emby / Plex Media Servers (rescued, stdlib-only)"
    )
    ...
    parser.add_argument("--ffprobe-path", default=os.environ.get("FFPROBE_PATH"), help="Custom path to ffprobe executable (or set FFPROBE_PATH env)")
    parser.add_argument("--recursive", "-r", action="store_true", help="Recursively inspect directories")
    parser.add_argument("--DryRun", "--dry-run", "--WhatIf", dest="dry_run", action="store_true", help="Preview files that WOULD be analyzed without invoking ffprobe; writes nothing")
    parser.add_argument("--out-file", default=None, help="Write JSON report to file (use with --DryRun to preview only)")
    ...
    if args.dry_run:
        print(f"[DryRun] Would analyze {len(files_to_inspect)} file(s) with ffprobe={analyzer.ffprobe_path!r}. No ffprobe invoked, no files written.")
        for _p in files_to_inspect[:50]:
            print(f"  would analyze: {_p}")
        if len(files_to_inspect) > 50:
            print(f"  ... and {len(files_to_inspect) - 50} more.")
        if args.out_file:
            print(f"[DryRun] Would write JSON report to: {args.out_file}")
        return 0
    ...
    if args.out_file:
        out_data = [r.to_dict() for r in results]
        payload = out_data if len(out_data) > 1 else (out_data[0] if out_data else {})
        Path(args.out_file).write_text(json.dumps(payload, indent=2), encoding="utf-8")
        print(f"Wrote JSON report to: {args.out_file}")
