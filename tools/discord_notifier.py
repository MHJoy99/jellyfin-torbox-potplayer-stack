#!/usr/bin/env python3
"""
Discord Webhook Notifier for MediaServer Ecosystem
==================================================
...
DEFAULT_BOT_USERNAME = "Nexus Media Assistant"
...
class DiscordWebhookClient:
    ...
    def send(
        ...
        headers = {
            ...
            "User-Agent": "MediaServer-DiscordNotifier/1.0",
        }
...
class TMDBPosterResolver:
    ...
    def fetch_media_details(
        ...
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "MediaServer-Notifier/1.0"})
            with urllib.request.urlopen(req, timeout=8) as resp:
                ...
                if media_type == "tv" and season is not None and episode is not None and tmdb_id:
                    ...
                    try:
                        ep_req = urllib.request.Request(ep_url, headers={"User-Agent": "MediaServer-Notifier/1.0"})
                ...
                # Fetch movie runtime if movie
                    ...
                    try:
                        m_req = urllib.request.Request(movie_url, headers={"User-Agent": "MediaServer-Notifier/1.0"})
...
class DiscordNotificationBuilder:
    """Constructs tailored Discord Embeds for each core MediaServer event type."""
    ...
    def build_media_ingest_embed(
        ...
        embed = DiscordEmbed(
            ...
            footer_text="Nexus Media Ecosystem • Automated Ingestion Engine",
        )
    ...
    def build_rclone_vfs_embed(
        ...
        embed = DiscordEmbed(
            ...
            color=color,
            author_name="Nexus Media Engine • Rclone VFS Monitor",
    ...
    def build_jellyfin_service_embed(
        ...
        server_name: str = "Nexus Media Hub",
    ...
    def build_db_backup_embed(
        ...
        embed = DiscordEmbed(
            ...
            color=color,
            author_name="Nexus Media Database Engine • Maintenance Task",
...
def inspect_system_vfs_storage(
    cache_dir: str = r"E:\MediaServer\cache\rclone_vfs",
    mount_drive: str = "X:",
) -> Dict[str, Any]:
...
def inspect_jellyfin_live_service(
    ...
    try:
        ...
        req = urllib.request.Request(ping_url, headers={"User-Agent": "MediaServer-Notifier/1.0"})
        with urllib.request.urlopen(req, timeout=4) as resp:
            data = json.loads(resp.read().decode("utf-8"))
            status_info["version"] = data.get("Version", "10.9.x")
            status_info["server_name"] = data.get("ServerName", "Nexus Media Hub")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Discord Webhook Notifier for MediaServer Ecosystem",
    ...
    parser.add_argument("--dry-run", action="store_true", help="Print Discord JSON payload without posting to webhook")
    ...
    p_vfs.add_argument("--cache-dir", default=r"E:\MediaServer\cache\rclone_vfs", help="VFS cache directory")
    ...
    p_jellyfin.add_argument("--server-name", default="Nexus Media Hub", help="Jellyfin Server Name")
    ...
    p_backup.add_argument("--log-path", default="E:\\MediaServer\\logs\\db_backup.log", help="Path to backup execution log")
    ...
    # -----------------------------------------------------------------------
        ...
        embed = DiscordEmbed(
            ...
            footer_text="Nexus Media Ecosystem • Status Alert",
        )
        if args.field:
