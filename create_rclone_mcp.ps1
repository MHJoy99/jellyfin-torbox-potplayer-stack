...
Write-Host "Creating multi-cloud storage management server for rclone..."

$mcpFolder = 'F:\Jellyfin\mcp-servers\rclone-storage'
New-Item -ItemType Directory -Force -Path $mcpFolder | Out-Null

$serverPy = Join-Path $mcpFolder 'server.py'
@'
import sys
import json
import subprocess
...
if __name__ == "__main__":
    main()
'@ | Out-File $serverPy -Encoding UTF8

Write-Host "Created server script at $serverPy"
